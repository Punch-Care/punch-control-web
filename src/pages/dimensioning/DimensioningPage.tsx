...
const createSchema = z.object({
  ...
  values: z.array(valueSchema).min(1, 'Adicione ao menos um valor'),
})

type FormData = z.infer<typeof createSchema>

function RecordRow({ record }: { record: DimensionRecord }) {
      ...
      <TableRow className="cursor-pointer hover:bg-muted/40" onClick={() => setOpen(!open)}>
        ...
        <TableCell>
          <Badge variant={allOk ? 'success' : 'destructive'}>
            {allOk ? 'OK' : `${nokCount} fora do limite`}
          </Badge>
        </TableCell>
        <TableCell className="text-muted-foreground text-sm">{record.values.length} parâmetros</TableCell>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-muted-foreground border-b">
                    <th className="text-left pb-2 font-medium">Parâmetro</th>
                    <th className="text-right pb-2 font-medium">Valor</th>
                    <th className="text-right pb-2 font-medium">Lim. Inf.</th>
                    <th className="text-right pb-2 font-medium">Lim. Sup.</th>
...
export function DimensioningPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (e: { response?: { data?: { message?: string } } }) =>
      toast.error(e.response?.data?.message ?? 'Erro ao criar registro'),
  })

  // deleteMutation available for future use if delete buttons are added
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold tracking-tight">Controle de Dimensionamento</h2>
          <p className="text-muted-foreground text-sm mt-0.5">Registro e acompanhamento de medições dimensionais</p>
        </div>
        {selectedSetId && (
      ...
      {selectedSet && (
        <div className="grid grid-cols-3 gap-3">
          <Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Total de Registros</p><p className="text-2xl font-bold">{records.length}</p></CardContent></Card>
          <Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Desvios Encontrados</p><p className={`text-2xl font-bold ${nokTotal > 0 ? 'text-red-500' : 'text-green-500'}`}>{nokTotal}</p></CardContent></Card>
          <Card><CardContent className="p-3"><p className="text-xs text-muted-foreground">Vida Útil</p><p className="text-2xl font-bold">{selectedSet.usefulValue.toFixed(0)}%</p></CardContent></Card>
        </div>
      )}
      ...
      {selectedSetId ? (
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Nenhum registro para este conjunto</TableCell></TableRow>
        ...
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            Selecione um conjunto para ver os registros de dimensionamento
          </CardContent>
        </Card>
      )}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>Nova Medição Dimensional</DialogTitle></DialogHeader>
                ...
                {fields.map((field, i) => (
                  <div key={field.id} className="grid grid-cols-12 gap-2 items-end">
                    <div className="col-span-4 space-y-1">
                      {i === 0 && <Label className="text-xs">Parâmetro</Label>}
                    ...
                    <div className="col-span-2 space-y-1">
                      {i === 0 && <Label className="text-xs">Lim. Inf.</Label>}
                      <Input type="number" step="0.001" placeholder="—" {...register(`values.${i}.lowerLimit`)} />
                    </div>
                    <div className="col-span-1 space-y-1">
                      {i === 0 && <Label className="text-xs">Lim. Sup.</Label>}
